...
export function generateFirstYearFromScratch(){
  ...
  const newPlayer = generateFirstYear(firstName, lastName, heightCm, abilitySigma, heightAdjustedPotentialSigma, personalitySigma);
...
export function generateFirstYear(firstName: string, lastName: string, heightCm : number, abilitySigma: number, potentialSigma: number, personalitySigma: number): Player {
  ...
  return {
    ...
    hittingOverall: hittingAbility.hittingOverall,
    fieldingOverall: hittingAbility.fieldingOverall,
    pitchingOverall: 0,
